...
IUSE="+cblas-external +deprecated eselect-cblas static-libs"

REQUIRED_USE="eselect-cblas? ( !cblas-external )"

BDEPEND="virtual/pkgconfig"
RDEPEND="cblas-external? ( virtual/cblas:= )"
DEPEND="${RDEPEND}
	eselect-cblas? ( app-eselect/eselect-cblas )
	"
...
pkg_pretend() {
	...
	if use cblas-external; then
		current_lib=$(eselect cblas show | cut -d' ' -f2 2>/dev/null)
